...
export default function SideBar() {
  ...
  return (
    <div className={`h-screen ${collapsed ? 'w-16' : 'w-64'} bg-gray-100 transition-all duration-300 flex flex-col shadow-lg`}>
      <div className="p-4 flex justify-between items-center border-b border-gray-200">
        ...
        {!collapsed && <span className="font-medium text-gray-800">Menu</span>}
      </div>
            ...
            <li key={index}>
              <Link 
                ...
                className={`flex items-center px-4 py-3 text-gray-700 hover:bg-blue-50 hover:text-blue-700 transition-colors ${
                  location.pathname === item.path ? 'bg-blue-100 text-blue-600 border-l-4 border-blue-500' : ''
                }`}
              >
                <div className={`text-xl ${collapsed ? 'mx-auto' : 'mr-4'}`}>{item.icon}</div>
